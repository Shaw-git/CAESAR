...
from model.run_gae_cuda import PCACompressor
...
class CAESAR:
    def __init__(self, 
        ...
        self.use_diffusion = True if diffusion_path is not None else False
        self.device = device
    ...
    def _load_models(self):
        if not self.use_diffusion:
            self._load_3d_compressor()
        else:
            self._load_2d_compressor()
            self._load_diffusion_model(self.diffusion_steps)

    def _load_3d_compressor(self):
        from .models import compress_modules3d_mid_SR as compress_modules
        print("Loading 3D Compressor with SR")
        ...
        state_dict = self.remove_module_prefix(torch.load(self.vae_path, map_location=self.device))
        model.load_state_dict(state_dict)
        self.compressor_v = model.to(self.device).eval()

    def _load_2d_compressor(self):
    ...
    def _load_diffusion_model(self, steps):
        ...
        state_dict = self.remove_module_prefix(torch.load(self.diffusion_path, map_location=self.device))
    ...
    def decompress(self, compressed):
        ...
        if self.use_diffusion:
            recons_data = self.decompress_caesar_d(compressed["latent"], compressed["shape"])
            recons_data = self.transform_shape(recons_data)
        else:
            recons_data = self.decompress_caesar_d(compressed["latent"], compressed["shape"])
            recons_data = self.transform_shape(recons_data)
    ...
    def decompress_caesar_d(self, all_compressed, shape):
    ...
    def decompress_caesar_v(self, all_compressed, shape):
